  ...
  return (
    ...
    <div className='flex'>
      <div className={`${open ? 'w-72' : 'w-20'} duration-300 bg-slate-800 flex flex-col h-screen px-4 py-2 pt-8 relative`}>
        <img className={`${open ? 'h-24' : 'h-12 w-12'} my-4`} src='account.svg' alt=''/>
        <img className={`-right-[0.85rem] top-10 cursor-pointer rounded-full h-10 absolute ${!open && 'rotate-180 h-8'}`} onClick={() => setopen(!open)} src='left.svg' alt=''/>
        ...
        <div className={`flex ${!open ? 'flex-col space-y-2' : 'flex-row space-x-6'} pb-8 border-b border-b-slate-400 justify-center`}>
          <button className='hover:bg-slate-400 p-2 rounded-xl'>
            <img className={`h-8 ${!open && 'w-min'}`} src='apps.svg' alt='' />
          </button>
          <button className='hover:bg-slate-400 p-2 rounded-xl'>
            <img className={`h-8 ${!open && 'w-min'}`} src='bell.svg' alt='' />
          </button>
          <button className='hover:bg-slate-400 p-2 rounded-xl'>
            <img className={`h-8 ${!open && 'w-min'}`} src='help.svg' alt='' />
          </button>
        </div>
